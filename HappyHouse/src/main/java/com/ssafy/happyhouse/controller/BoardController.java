...
public class BoardController {
	...
	@ApiOperation(value = "회원 게시판 글목록", notes = "회원이 작성한 모든 게시글의 정보를 반환한다.", response = List.class)
	@GetMapping("/user/{userid}")
	public ResponseEntity<List<BoardDto>> listUserArticle(@PathVariable("userid") @ApiParam(value = "게시글을 얻기위한 회원 아이디.", required = true) String userid) throws Exception {
		logger.info("listUserArticle - 호출");

		return new ResponseEntity<List<BoardDto>>(boardService.listUserArticle(userid), HttpStatus.OK);
	}
	...
	public ResponseEntity<Map<String, Object>> listArticle(@ApiParam(value = "게시글을 얻기위한 부가정보.", required = true) BoardParameterDto boardParameterDto) throws Exception {
		...
		return new ResponseEntity<Map<String, Object>>(boardService.listArticle(boardParameterDto), HttpStatus.OK);
	}
